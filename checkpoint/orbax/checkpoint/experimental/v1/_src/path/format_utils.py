...
def validate_pytree_checkpoint(path: path_types.PathLike):
  ...
  if not (path / PYTREE_CHECKPOINTABLE_KEY).exists():
    raise FileNotFoundError(
        ...
        f' (called "{PYTREE_CHECKPOINTABLE_KEY}"). Please try inspecting the'
        ' checkpointable metadata using `ocp.checkpointables_metadata()` or try'
        ' loading the checkpoint using `ocp.load_checkpointables()`.'
    )
